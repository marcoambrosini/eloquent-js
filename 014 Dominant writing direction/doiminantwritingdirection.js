...
function dominantDirection(str) {
	...
	for (let i = 0; i < str.length; i++) {
		//filter is going to find out the only object in the array in which the ith character of the string's value is present 
		let newarr = SCRIPTS.filter(n => SCRIPTS[n].ranges.filter(o => SCRIPTS[n].ranges[o].some(x => x === str[i].codePointAt(0))));
		if (newarr.direction = "ltr") {
			leftToRight++;
		} else if (newarr.direction = "rtl") {
			rightToLeft++;
		} else if (newarr.direction = "ttb") {
			topToBottom++;
		}
	}
	if (leftToRight > topToBottom && leftToRight > rightToLeft) {
		return 'Left to rigth';
	} else if (topToBottom > leftToRight && topToBottom > rightToLeft) {
		return 'Top to Botoom';
	} else if (rightToLeft > leftToRight && rightToLeft > topToBottom) {
		return 'Right to left';
	} else if (leftToRight === rightToLeft === topToBottom) {
		return 'Equal';
	} else if (leftToRight === rightToLeft > topToBottom) {
		return 'Left to right and right to left';
	} else if (leftToRight < rightToLeft === topToBottom) {
		return 'Right to left and top to bottom';
	} else if (leftToRight === topToBottom > rightToLeft) {
		return 'Left to right and top to bottom';
	}
